...
//#pragma HLS pipeline
			for (int row = 0; row < POOL_1_OUTPUT_WH; row++){
				for (int col = 0; col < POOL_1_OUTPUT_WH; col++){
					...
					dst[batch][depth][row][col] = tanhf(value);//[(batch * POOL_1_TYPE + depth)*POOL_1_OUTPUT_SIZE + row * POOL_1_OUTPUT_WH + col] = tanhf(value);
				}
			}
		}
	}
...
//#pragma HLS pipeline
			for (int row = 0; row < POOL_2_OUTPUT_WH; row++){
				for (int col = 0; col < POOL_2_OUTPUT_WH; col++){
					...
					hw_fixed weight = pool_kernel[depth]*(hw_fixed)0.25;
					value *= weight;
					dst[batch][depth][row][col] = tanhf(value);//[(batch * POOL_2_TYPE + depth)*POOL_2_OUTPUT_SIZE + row * POOL_2_OUTPUT_WH + col] = tanhf(value + pool_bias[depth]);
				}
			}
		}
	}
